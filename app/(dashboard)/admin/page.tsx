...
export default async function AdminPage() {
  ...
  const [articles, brew, userCount] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <StatCard label="Total Articles" value={articleCount} icon={<FileText size={16} />} color="#F45B69" />
              ...
              {articles.map(article => (
                <tr key={article.id} className="border-b border-[#3A3A3A] last:border-0 hover:bg-[#2E2E2E] transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/articles/${article.id}`} className="text-[#F0F0F0] hover:text-[#F45B69] transition-colors line-clamp-1">{article.title}</Link>
                  </td>
