...
export async function trackRead(userId: string, articleId: string): Promise<void> {
  ...
  await supabase.from("user_reads").upsert({ user_id: userId, article_id: articleId }, { onConflict: "user_id,article_id" });
}

export async function getUserReadIds(userId: string, limit = READ_HISTORY_LIMIT): Promise<string[]> {
  if (isDemoMode()) return [];
  const supabase = await createClient();
  const { data } = await supabase.from("user_reads").select("article_id").eq("user_id", userId).order("read_at", { ascending: false }).limit(limit);
...
export async function getUserSaveIds(userId: string): Promise<Set<string>> {
  ...
  const supabase = await createClient();
  const { data } = await supabase.from("user_saves").select("article_id").eq("user_id", userId);
...
export async function isSaved(userId: string, articleId: string): Promise<boolean> {
  ...
  const supabase = await createClient();
  const { data } = await supabase.from("user_saves").select("article_id").eq("user_id", userId).eq("article_id", articleId).single();
  return !!data;
}
