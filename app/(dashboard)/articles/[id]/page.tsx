...
import { formatDate, categoryChipStyle } from "@/lib/utils";
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <span className="flex items-center gap-1 text-[#6A6A6A] text-xs"><Clock size={11} />{formatDate(article.published_at)}</span>
          ...
          <span className="flex items-center gap-1 text-[#6A6A6A] text-xs ml-auto"><Eye size={11} />{article.read_count} reads</span>
        </div>
        ...
        {allTags.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap mb-6">
            ...
            {allTags.map(tag => (
              <span key={tag} className="text-xs bg-[#2E2E2E] border border-[#3A3A3A] text-[#A0A0A0] px-2.5 py-1 rounded-lg">{tag}</span>
            ))}
          </div>
        )}
