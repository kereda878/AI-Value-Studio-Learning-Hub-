  ...
  return (
    <div className="min-h-screen bg-[#1A1A1A] flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-md bg-[#242424] rounded-2xl border border-[#3A3A3A] p-8">
        ...
        {error && (
          <div className="mb-4 px-4 py-3 rounded-lg bg-[#F45B69]/10 border border-[#F45B69]/30 text-[#F45B69] text-sm">{error}</div>
        )}
        {message && (
          <div className="mb-4 px-4 py-3 rounded-lg bg-[#D4956A]/10 border border-[#D4956A]/30 text-[#D4956A] text-sm">{message}</div>
        )}

        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
            label="Email" type="email" required value={email}
            onChange={e => setEmail(e.target.value)} placeholder="you@genpact.com"
            icon={<Mail size={15} />}
          />
          <div>
            <Input
            ...
            <button type="button" onClick={() => setShowPassword(!showPassword)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6A6A6A] hover:text-[#A0A0A0]"
            >
